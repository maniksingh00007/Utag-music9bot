...
from TEAMXMUSIC.utils.stream.autoclear import auto_clean
from TEAMXMUSIC.utils.thumbnails import get_thumb
from TEAMXMUSIC.utils.errors import capture_internal_err, send_large_error
...
class Call:
    def __init__(self):
        self.userbot1 = Client(
            "TEAMXXAssis1", config.API_ID, config.API_HASH, session_string=config.STRING1
        ) if config.STRING1 else None
        self.one = PyTgCalls(self.userbot1) if self.userbot1 else None

        self.userbot2 = Client(
            "TEAMXXAssis2", config.API_ID, config.API_HASH, session_string=config.STRING2
        ) if config.STRING2 else None
        self.two = PyTgCalls(self.userbot2) if self.userbot2 else None

        self.userbot3 = Client(
            "TEAMXXAssis3", config.API_ID, config.API_HASH, session_string=config.STRING3
        ) if config.STRING3 else None
        self.three = PyTgCalls(self.userbot3) if self.userbot3 else None

        self.userbot4 = Client(
            "TEAMXXAssis4", config.API_ID, config.API_HASH, session_string=config.STRING4
        ) if config.STRING4 else None
        self.four = PyTgCalls(self.userbot4) if self.userbot4 else None

        self.userbot5 = Client(
            "TEAMXXAssis5", config.API_ID, config.API_HASH, session_string=config.STRING5
